...
| 우선순위 | 항목 | 핵심 검증 |
| --- | --- | --- |
| 즉시 | 1-1 API 키 버전 관리 노출, 1-2 미사용 Daum 키 정리, 2-1 KSP 버전 수정 | `git grep` secret 확인, `./gradlew :app:assembleDemoDebug :app:assembleProdDebug` |
| 단기 | 1-3 cleartext 범위 축소, 3-2 cache pruning 연결, 4-1 retry catch 범위 축소, 5-1 미emit 이벤트 연결 | 관련 data/feature/app 단위 테스트 |
| 중기 | 2-4 release minification, 3-1 UI state 재매핑 축소, 3-3 watchlist 최신 캐시 조회 개선, 7-1 브랜드 라벨 단일 출처 | assemble, repository/ViewModel/UI model 테스트 |
| 장기 | 2-5 테스트 컨벤션 플러그인, 6-1/6-2 deprecated API 교체, 9-1/9-2 theme/string resource 정리, 10-1 datastore 의존 방향 재검토 | 전체 회귀 세트 또는 관련 모듈 matrix |
...
두 키가 프로젝트 루트 `gradle.properties`에 평문으로 저장되어 있습니다. 이 파일은 git 추적 대상이므로 레포지터리 접근 권한을 가진 사용자가 키를 볼 수 있고, 이미 커밋된 적이 있다면 git 히스토리에도 남습니다.
...
**완료 기준:**
- `git grep -n "apikey=.*[A-Za-z0-9]"`가 문서 예시나 placeholder 외 실제 키를 찾지 못합니다.
...
**검증:** `git diff --check`, `./gradlew :app:assembleDemoDebug :app:assembleProdDebug :app:testProdDebugUnitTest`

---

### 1-2. 미사용 Daum API 키 고아 `[위험]`

**파일:** `gradle.properties`

**소유:** runtime setup, 외부 지도 handoff 문서

`daum.apikey`는 루트 `gradle.properties`에 존재하지만 현재 `buildConfigField`, Kotlin 소스, Gradle task에서 참조되지 않습니다. 지도 handoff는 앱별 URI와 package name으로 처리되며 Daum/Kakao API 키를 사용하지 않습니다.
...
### 1-3. HTTP cleartext 트래픽 전역 허용 `[품질]`
...
Opinet API가 HTTP endpoint를 요구한다면 일부 cleartext 허용은 불가피할 수 있습니다. 다만 현재 설정은 앱 전체 네트워크 트래픽에 cleartext를 허용합니다.
...
<network-security-config>
    <domain-config cleartextTrafficPermitted="true">
        <domain includeSubdomains="true">opinet.co.kr</domain>
    </domain-config>
</network-security-config>
```

**완료 기준:** Opinet 통신만 cleartext 예외가 되고 다른 도메인은 Android 기본 정책을 따릅니다.

**검증:** `./gradlew :app:processDemoDebugMainManifest :core:network:test`

---

### 1-4. Android 백업 규칙 미설정 `[품질]`
...
`AndroidManifest.xml`에는 `android:allowBackup="true"`가 설정되어 있지만 backup/data extraction 규칙은 샘플 주석 위주입니다. Room DB에는 캐시, 가격 히스토리, watchlist가 있고 DataStore에는 사용자 설정이 있습니다.
...
**완료 기준:** 백업 대상과 제외 대상이 코드와 문서에서 일치합니다.

**검증:** `./gradlew :app:testDemoDebugUnitTest`

---

### 1-5. `BuildConfig`에 주입된 prod API 키는 APK에서 추출 가능 `[품질]`
...
`opinet.apikey`가 `BuildConfig.OPINET_API_KEY`로 들어가는 구조는 모바일 클라이언트 배포물에서 키가 완전히 비밀일 수 없다는 한계를 가집니다. 이 항목은 현재 포트폴리오/reference 앱에서는 수용 가능한 단순화일 수 있지만, 공개 배포 또는 quota 비용이 큰 API라면 별도 정책이 필요합니다.
...
### 2-1. KSP 버전과 Kotlin 버전 쌍 불일치 `[버그]`
...
KSP Gradle plugin은 Kotlin compiler와 맞는 release line을 사용해야 합니다. 현재 값은 Kotlin 버전과 KSP plugin 버전 쌍이 맞는지 즉시 확인이 필요합니다. 이 항목은 빌드 전체를 막을 수 있으므로 실제 수정 전 KSP release tag와 Gradle resolution 결과를 다시 확인해야 합니다.
...
`proguard-rules.pro`가 존재하지만 release build에서 R8/ProGuard가 실행되지 않습니다. 포트폴리오 앱이어도 release artifact 크기와 역공학 난이도 측면에서는 축소를 검증할 가치가 있습니다.
...
### 3-1. UI state combine이 매 emission마다 전체 목록을 재매핑 `[성능]`
...
`preferences`, `locationStateMachine.state`, `transientState`, `searchOrchestrator.searchResult`, `searchOrchestrator.blockingFailure` 중 하나라도 emission이 발생하면 `result.stations.map(::StationListItemUiModel)`이 전체 목록을 새로 매핑합니다. 목록이 커지거나 위치 availability가 자주 emit되면 불필요한 객체 생성이 늘 수 있습니다.
...
### 3-2. `pruneOlderThan` 미호출로 cache snapshot이 누적됨 `[성능]`
...
`StationCacheDao.pruneOlderThan()`은 구현되어 있지만 프로덕션 repository 경로에서 호출되지 않습니다. 가격 히스토리는 `keepLatestTenByStationAndFuelType()`로 정리되지만, `station_cache`와 `station_cache_snapshot`은 오래된 버킷이 계속 남을 수 있습니다.
...
### 3-3. watchlist 최신 캐시 조회가 Kotlin 정렬에 의존 `[성능]`

**파일:** `core/database/src/main/kotlin/com/gasstation/core/database/station/StationCacheDao.kt`, `data/station/src/main/kotlin/com/gasstation/data/station/DefaultStationRepository.kt`

**소유:** `data:station`, `core:database`

`observeLatestStationsByIds()`는 SQL에서 정렬된 전체 row를 반환하고, repository의 `latestByStationId()`가 다시 정렬 후 `groupBy`로 station별 최신 row를 선택합니다. watchlist 규모가 커지면 불필요한 정렬과 allocation이 생깁니다.
...
### 4-1. `StationRetryPolicy`가 retry 실패에서 모든 `Throwable`을 catch `[버그]`

**파일:** `data/station/src/main/kotlin/com/gasstation/data/station/StationRetryPolicy.kt`

**소유:** `data:station`

retry 시도 중 `CancellationException`은 별도로 전파하지만 그 외는 `Throwable`로 잡아 `RetryAttempted(succeeded=false)`를 기록한 뒤 다시 던집니다. 예기치 않은 `Error` 계열까지 retry 실패 이벤트로 기록될 수 있고, "일시적 refresh 실패 재시도"라는 정책 범위보다 catch 범위가 넓습니다.
...
### 5-1. `SearchRefreshed`, `CompareViewed`, `ExternalMapOpened` 이벤트 미emit `[미완성]`
...
세 이벤트는 domain 계약과 Logcat 매핑에 존재하지만 현재 프로덕션 코드에서 emit되지 않습니다. `docs/state-model.md`와 `docs/architecture.md`도 이 사실을 이미 설명합니다.

| 이벤트 | 기대 시점 | 현재 상태 | 구현 시 주의 |
| --- | --- | --- | --- |
| `SearchRefreshed` | refresh 성공 후 | 미emit | active query와 stale 여부를 같은 source에서 계산 |
| `CompareViewed` | watchlist 비교 화면 진입 또는 첫 데이터 표시 | 미emit | 중복 emit 방지, count 기준 명확화 |
| `ExternalMapOpened` | 외부 지도 handoff 요청 또는 실제 startActivity 성공 | 미emit | `StationListEffect.OpenExternalMap`에 stationId가 없어 계약 보강 필요 |

**권장 조치:**
- 먼저 event 의미를 "요청됨"인지 "성공적으로 외부 앱을 열었음"인지 정합니다.
- `ExternalMapOpened`를 실제 성공 기준으로 남기려면 `ExternalMapLauncher.open()`이 결과를 반환하거나 app 계층에서 logger를 주입받는 설계를 검토합니다.
- `CompareViewed`는 watchlist route 또는 ViewModel에서 한 번만 emit되도록 lifecycle 기준을 잡습니다.

**완료 기준:** domain event variant가 실제 사용자 플로우에서 emit되고, 미emit 상태라고 적은 문서가 함께 갱신됩니다.
...
### 6-2. `Geocoder.getFromLocation` 동기 API API 33 deprecated `[품질]`
...
현재 I/O dispatcher에서 실행되므로 메인 스레드 블로킹은 피하고 있지만, API 33+에서는 listener 기반 비동기 overload가 권장됩니다.

**권장 조치:** API 33+에서는 callback overload를 coroutine으로 감싸고, 그 이하에서는 기존 동기 API를 fallback으로 사용합니다.

**완료 기준:** 주소 라벨 정규화와 error/cancellation 전파 의미가 기존 테스트와 동일합니다.
...
같은 `Brand.RTX`/`BrandFilter.RTX`가 화면 위치에 따라 다르게 읽힙니다. `BrandFilter.ETC`도 `자가상표`로 표시되어 station card의 `Brand.RTX`와 의미가 겹칩니다.
...
### 8-3. `pruneOlderThan` repository 호출 경로 테스트 부재 `[테스트]`

**파일:** `core/database`, `data/station` tests

**소유:** `data:station`, `core:database`

DAO의 prune 동작만으로는 repository가 실제 refresh 성공 후 정리를 호출하는지 보장할 수 없습니다. 3-2를 수정할 때 repository 레벨 테스트가 필요합니다.
...
아래 항목을 실제로 수정하면 코드와 함께 문서도 점검합니다.

- 1-1/1-5: README 실행 모드와 secret 안내
- 1-3: architecture/network 설명
- 3-2/3-3/8-2/8-3: `docs/offline-strategy.md`, `docs/state-model.md`
- 5-1: `docs/state-model.md`, `docs/architecture.md`
- 7-1/9-1/9-3: README screenshot/demo story, 디자인 관련 문서
