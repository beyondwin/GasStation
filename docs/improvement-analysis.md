...
### 2-5. 테스트 컨벤션 플러그인 `[빌드]` - 완료

**파일:** `build-logic/convention/*`, 각 모듈 `build.gradle.kts`

**소유:** `build-logic/convention`

`gasstation.android.library`가 Android library 공통 unit test 의존성(`junit`, `kotlinx-coroutines-test`, `androidx.test:core`, `robolectric`)을 공급하고, `gasstation.android.library.compose`가 Compose library UI test/debug 의존성(Compose test BOM, UI test JUnit4, UI tooling, UI test manifest)을 공급하도록 정리했습니다. Android library module build file에는 Turbine, `kotlin.test`, project test dependency, androidTest smoke dependency처럼 모듈별로 필요한 의존성만 남겼습니다.

**후속 조치:** JVM-only/domain/tool module test dependency는 이번 Android library convention 정리 범위에서 제외했습니다. 별도 JVM test convention이 필요하면 독립 작업으로 다룹니다.
...
### 6-1. `window.statusBarColor` API 35 deprecated `[완료됨]`

**파일:** `app/src/main/java/com/gasstation/MainActivity.kt`, `core/designsystem/src/main/kotlin/com/gasstation/core/designsystem/Theme.kt`
...
`Theme.kt`의 deprecated status bar 색상 직접 쓰기와 local suppress를 제거했습니다. 런타임 system chrome 정책은 `app`의 `MainActivity`가 `enableEdgeToEdge()`와 `SystemBarStyle`로 소유하고, `core:designsystem`은 `GasStationThemeDefaults.statusBarStyle` 토큰과 theme provider 역할만 유지합니다.

**조치:** `MainActivity`가 `GasStationThemeDefaults.statusBarStyle.backgroundColor`로 status/navigation bar style을 적용합니다. `GasStationTheme`는 Activity window를 mutate하지 않습니다.

**완료 기준:** API 35 deprecation suppress가 제거되고, demo/prod assemble과 emulator smoke에서 yellow/black/white identity와 가격 우선 station card 위계를 유지합니다.

**검증:** `./gradlew :app:testDemoDebugUnitTest --tests com.gasstation.SystemBarPolicyTest :core:designsystem:testDebugUnitTest :app:testDemoDebugUnitTest :app:assembleDemoDebug :app:assembleProdDebug`, `./gradlew :app:installDemoDebug`, Pixel_9 emulator screenshot smoke.
...
**완료 기준:** callback error는 `LocationAddressLookupResult.Error`, 빈 성공 결과는 `Unavailable`, cancellation은 전파되고 주소 라벨 정규화 의미는 유지됩니다. API 33+ device-backed callback smoke는 `AndroidAddressResolverDeviceTest`가 terminal domain result만 검증하고, provider별 주소 문자열에는 의존하지 않습니다.

**검증:** `./gradlew :core:location:testDebugUnitTest :feature:station-list:testDebugUnitTest`; device smoke는 API 33+ 기기/에뮬레이터가 있을 때 `./gradlew :core:location:connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.gasstation.core.location.AndroidAddressResolverDeviceTest`
...
### 9-1. 다크 모드 지원이 부분적임 `[부분 처리: cleanup/review 완료]` `[접근성]`
...
**Task 5 cleanup/review:** theme/token inventory found app XML launch chrome in
`app/src/main/res/values/colors.xml` and `app/src/main/res/values-v31/themes.xml`,
and Compose theme tokens in `core:designsystem`. `ic_launcher_background`,
`ic_launcher_foreground_fill`, `windowSplashScreenBackground`, and
`windowSplashScreenAnimatedIcon` stay app-owned because they are Android launcher
and splash resources, not Compose design tokens. `ColorYellow`, black/white
anchors, semantic surfaces, typography, spacing, corner, stroke, and
`GasStationThemeDefaults.statusBarStyle` stay in `core:designsystem`. No token was
moved and no visual redesign was performed; the broader dark-mode semantic-role
migration remains a separate UI/accessibility task.

---

### 9-2. 사용자 표시 문자열이 Kotlin에 하드코딩됨 `[부분 처리: cleanup/review 완료]` `[접근성]`
...
**Task 5 cleanup/review:** string inventory found no hard-coded Korean string
literal in `app/src/main`; the manifest already uses `@string/app_name`, and
`app/src/main/res/values/strings.xml` already contains only the app-owned
`app_name`. Feature-local Compose strings remain in `feature:*` modules until
each feature owns an Android resource strategy. Moving station-list, settings,
or watchlist screen copy into `app` or `core:designsystem` would invert
ownership. `core:designsystem/BrandLabels.kt` remains the shared owner for
brand/filter labels because those labels are shared brand vocabulary, not
feature-specific screen copy.
...
### 10-1. `core:datastore`가 `domain:settings`에 의존 `[완료됨]`

**파일:** `core/datastore`, `domain/settings`, `data/settings`, `docs/module-contracts.md`

**소유:** `core:datastore`, `domain:settings`, `data:settings`

`core:datastore -> domain:settings` 의존 예외를 제거했습니다. `core:datastore`는 `StoredUserPreferences` storage-local DTO와 serializer만 소유하고, enum name은 primitive string으로 저장합니다. `data:settings`가 storage DTO를 `domain:settings.model.UserPreferences`로 매핑하며, 알 수 없는 enum name은 domain default로 fallback합니다.

**조치:** `domain:settings` public model이 `core:model` enum을 노출하므로 `domain/settings/build.gradle.kts`는 `core:model`을 `api`로 게시합니다. `data:settings`는 기존 `domain:settings`/`core:datastore` 의존 경계 안에서 mapper를 소유합니다.

**완료 기준:** `core:datastore` runtime classpath에서 `domain:settings`가 사라지고 설정 저장, demo preference reset, settings feature 테스트가 유지됩니다.

**검증:** `./gradlew :domain:settings:test :core:datastore:testDebugUnitTest :data:settings:testDebugUnitTest :feature:settings:testDebugUnitTest`, `./gradlew :app:testDemoDebugUnitTest --tests com.gasstation.startup.DemoSeedStartupHookTest`
