...
| `core:location` | `AndroidForegroundLocationProviderTest`, `DefaultLocationRepositoryTest`, `LocationAvailabilityFlowTest`, `LocationPermissionStateTest` | Android 위치 조회, domain location 구현, availability broadcast 반영 |
...
| `feature:station-list` | `StationListViewModelTest`, `StationListScreenTest`, `StationListBannerModelTest`, `GpsAvailabilityMonitorTest` | 상태 전이, effect, stale 배너, route lifecycle 기반 availability 관찰과 권한/GPS recovery |
