...
Repository/default CI/ordinary-local outer convention `Test`의 blocking timeout은 50분입니다. Reviewed Round-21 duration ledger는 lane 배치 입력으로 유지하지만 hosted main CI에서 27분과 35분 상한이 동일 52-owner/90-test inventory를 중단한 증거를 반영합니다. Nested TestKit module `Test` timeout은 15분이며 convention suite는 retry, shard, skip 없이 같은 test inventory를 실행합니다.
