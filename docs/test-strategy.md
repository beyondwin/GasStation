...
Local Linux metadata capture의 repository/default/CI/ordinary-local 및 nested TestKit timeout은 15분입니다. Sole sealed Colima orchestrator의 outer build-logic convention Test만 reviewed exact property, outer-only marker environment, canonical nonsymlink mode-`0600` source/policy/attempt/task-bound marker가 모두 일치할 때 30분을 사용합니다. 두 mode 모두 `maxParallelForks=5`, 전체 90 tests를 고정하고 retry/shard/skip으로 완화하지 않습니다. 30분은 진단과 complete-run window일 뿐 원인이나 PASS 추론이 아닙니다.

실패 또는 timeout에서는 listener가 stable owned root에 case XML, worker event, bounded exception/nested output을 점진적으로 기록하고 Gradle Test finalizer가 temp TestKit tree 삭제 전에 worker stream과 canonical manifest를 닫고 재해시합니다. 그 뒤 outer exporter가 class/method, exact decimal duration/status/count, redacted exception/nested-log extract를 canonical failure summary로 봉인합니다. 이 summary와 extract의 owner/path/hash/size/truncation, outer-30/repository+nested-15 marker triple 또는 source/policy/attempt/command binding이 빠지거나 달라지면 diagnostic collection 자체가 실패하며 partial test 결과를 성공으로 바꾸지 않습니다.
