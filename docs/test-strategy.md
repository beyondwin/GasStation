...
| `domain:*` | `BrandFilterTest`, `StationPriceDeltaTest`, `StationQueryCacheKeyTest`, `DomainContractSurfaceTest`, `UserPreferencesTest` | 순수 규칙, 계약 표면, 캐시 키 계산 |
...
| `core:location` | `AndroidForegroundLocationProviderTest`, `LocationPermissionStateTest` | 위치 조회 결과 분기 |
...
| `feature:station-list` | `StationListViewModelTest`, `StationListScreenTest`, `StationListBannerModelTest`, `GpsAvailabilityMonitorTest` | 상태 전이, effect, stale 배너, GPS 관찰 |
