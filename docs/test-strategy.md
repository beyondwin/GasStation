...
| `feature:station-list` | `StationListViewModelTest`, `StationListScreenTest`, `StationListBannerModelTest`, `GpsAvailabilityMonitorTest` | 상태 전이, effect, stale 배너, 주소 컨텍스트 표시, 브랜드 아이콘 중심 카드, route lifecycle 기반 availability 관찰과 권한/GPS recovery |
| `feature:settings` | `SettingsViewModelTest`, `SettingsScreenTest`, `SettingsSectionTest` | 설정 요약/상세 화면 계약 |
| `feature:watchlist` | `WatchlistViewModelTest`, `WatchlistScreenTest`, `WatchlistItemUiModelTest` | 북마크 비교 화면 상태와 표시 |
