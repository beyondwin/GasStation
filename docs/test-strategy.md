...
Local Linux metadata capture의 convention Test task는 timeout 15분, `maxParallelForks=5`, 전체 90 tests를 고정하고 retry/shard/skip으로 완화하지 않습니다. 실패 또는 timeout에서는 temp TestKit tree 삭제 전에 outer class/method와 worker, exact decimal duration/status/count, redacted exception/nested-log extract를 canonical failure summary로 봉인합니다. 이 summary와 extract의 owner/path/hash/size/truncation 또는 source/policy/attempt/command binding이 빠지거나 달라지면 diagnostic collection 자체가 실패하며 partial test 결과를 성공으로 바꾸지 않습니다.
