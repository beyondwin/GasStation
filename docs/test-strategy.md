...
GasStation의 테스트는 "현재 공식 경로인 `demo`와 `prod`가 계속 성립하는가"를 확인하는 데 집중합니다. 정확한 실행 명령은 `docs/verification-matrix.md`를 보고, 이 문서에서는 어떤 층을 어떤 테스트로 막고 있는지 설명합니다.
...
| `core:location` | `AddressLabelFormatterTest`, `AndroidForegroundLocationProviderTest`, `DefaultLocationRepositoryTest`, `LocationAvailabilityFlowTest`, `LocationPermissionStateTest` | Android 위치 조회, 주소 라벨 정규화, domain location 구현, availability broadcast 반영 |
| `core:datastore` | `UserPreferencesSerializerTest`, `AndroidUserPreferencesDataSourceTest` | 커스텀 serializer와 DataStore 업데이트 |
| `core:designsystem` | `GasStationThemeDefaultsTest`, `GasStationThemeTokensTest`, `ChromeContractsTest`, `BrandIconTest` | 브랜드 anchor 색상, typography/spacing token, metric/supporting-info/row/status/guidance chrome 계약 |
...
| `feature:station-list` | `StationListViewModelTest`, `StationListScreenTest`, `StationListBannerModelTest`, `GpsAvailabilityMonitorTest` | 상태 전이, effect, stale/approximate guidance, 주소 컨텍스트 표시, 가격 우선 카드, 긴 역명/가격/유종 clipping 방지, route lifecycle 기반 availability 관찰과 권한/GPS recovery |
