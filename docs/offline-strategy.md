...
다만 목록 화면의 active query는 브랜드 필터와 정렬도 포함합니다. 현재 좌표가 유지된 상태에서 조건이 바뀌면 feature의 `RefreshCoordinator`가 새 active query로 refresh를 다시 요청합니다. 캐시 키에는 브랜드/정렬이 없으므로 기존 스냅샷은 즉시 재사용해 필터/정렬할 수 있고, 원격 refresh가 성공하면 같은 버킷 스냅샷이 최신 데이터로 교체됩니다.
