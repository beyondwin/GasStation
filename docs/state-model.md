...
GasStation은 상태를 다섯 층으로 나눠서 보면 가장 덜 헷갈립니다. 이 문서는 각 상태가 어디서 만들어지고 얼마나 오래 유지되는지 설명합니다.
...
- `demo`에서는 `DemoLocationOverride`가 좌표를 바로 공급하고 권한 상태도 사실상 허용으로 취급합니다.
- `prod`에서는 `ForegroundLocationProvider`가 `LocationLookupResult`로 결과를 돌려주므로, ViewModel이 success, timeout, unavailable, permission denied, error를 구분해 처리합니다.
- GPS 상태는 resume 시점 재확인 한 번이 아니라 `gpsAvailabilityFlow()`를 통해 화면이 foreground인 동안 계속 반영됩니다.
...
- 사용자가 항목을 선택하면 곧바로 `SettingsRepository.updateUserPreferences()`가 호출됩니다.
