...
3. 구현 전 기준 `StationListViewModel`(522줄)의 책임을 분리한다.
...
계획 작성 시점에는 `core:datastore`, `core:network`, `core:designsystem` 세 모듈이 `domain:station`에 의존했다. Clean Architecture에서 core 레이어는 domain보다 아래에 위치하므로 domain을 참조해서는 안 된다.
...
원인은 당시 `domain:station/model/`에 있던 6개 enum이 실질적으로 **공유 어휘(shared vocabulary)** 역할을 했기 때문이다. 이 enum들은 도메인 비즈니스 로직이 아니라, 검색 조건과 설정을 표현하는 값 타입이다.

### Enums to Move

| Enum | 계획 당시 위치 | 영향 파일 수 | 비고 |
|------|----------|------------|------|
...
계획 작성 시점의 실측 기준으로 51개 `.kt` 파일, 129개 import occurrence가 변경 대상이었다.

### Target Location

`core:model`의 새 패키지 `com.gasstation.core.model`에 둔다.

계획 작성 시점의 `core:model`은 `Coordinates`, `DistanceMeters`, `MoneyWon` 세 가지 값 객체만 가지고 있었다. 이동 대상 enum들도 본질적으로 값 타입이므로 같은 패키지에 자연스럽게 들어간다.
...
계획 작성 시점의 `domain:station/build.gradle.kts`는 `implementation(project(":core:model"))`이므로, `domain:station`을 통해 enum에 접근하던 모듈은 직접 `core:model` 의존을 추가해야 했다. 단, 대부분의 모듈은 이미 `core:model`을 의존하고 있었으므로 추가 작업은 최소화된다.
...
계획 작성 시점에는 `UserPreferences`가 `domain:station`의 enum 5개를 import했다:
...
이로 인해 당시 `domain:settings`가 `domain:station`에 의존하고, `core:datastore`가 `domain:settings`를 통해 간접적으로 `domain:station`까지 의존했다.
...
정리 전에는 `StationListViewModel`(522줄)이 다음 책임을 모두 소유했다:
...
- `StationListViewModel`: ~250줄 (정리 전 522줄에서 약 52% 감소)
...
정리 전에는 네트워크 실패가 발생하면 즉시 UI에 전파됐다. `Timeout`이나 `Network` 실패는 일시적이므로, 1회 자동 재시도만으로도 사용자 경험이 크게 개선된다.
...
정리 전에는 `StationEventLogger`가 분석 이벤트(검색, watch, 비교, 외부 지도)만 기록했다. 에러 자체를 구조화해서 기록하는 계약이 없어, 프로덕션에서 실패 원인을 진단하기 어려웠다.
...
**import 경로 변경 (계획 작성 시점 실측: 51개 `.kt` 파일, 129개 import occurrence):**
