...
앱은 강남역 2번 출구 고정 좌표를 현재 위치로 사용한다. 사용자가 반경을 `3km`, `4km`, `5km`로 바꾸거나 유종을 `휘발유`, `경유`, `고급휘발유`, `등유`, `LPG`로 바꿔도, 앱은 이미 로컬 DB에 적재된 실제 데이터 기반 스냅샷을 사용해 목록을 보여준다.
...
- `OpinetService`
- 현재 `StationRemoteDataSource`가 사용하는 `KATEC <-> WGS84` 로컬 좌표 변환과 원격 매핑 규칙
...
- 반경 3/4/5km와 유종 전체를 바꿔도 로컬 시드 결과가 나온다.
