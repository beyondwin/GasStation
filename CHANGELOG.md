...
- live 문서를 목적별 지도로 줄이고, 같은 설명을 여러 곳에 반복하지 않도록 정리했다.
- 아키텍처에 쉬운 말, 한눈에 레이어, 데이터 흐름을 보강하고 지금 문서의 계획 번호 잔재를 현재 소유 문서로 바꿨다.
- GitHub Ubuntu 24.04 후속 `20260907.300.1`을 recapture 없이 허용한다.
