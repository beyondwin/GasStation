...
from device.gmd_processes import discover_processes, introduced_processes, read_snapshot

def terminate_introduced(
    baseline: list[dict], *, wait_seconds: float = 20
) -> tuple[list[dict], list[dict], list[dict], list[dict]]:
    observed = discover_processes()
    owned = introduced_processes(baseline, observed)
    killed: list[dict] = []
    failures: list[dict] = []
    for process in owned:
        current_identities = {
            (item["pid"], item["executable"])
            for item in discover_processes()
        }
    ...
    live = introduced_processes(baseline, discover_processes())
...
def main() -> int:
    ...
    observed, killed, failures, live = terminate_introduced(baseline)
