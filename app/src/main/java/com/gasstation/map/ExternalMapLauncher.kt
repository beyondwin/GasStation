...
class IntentExternalMapLauncher @Inject constructor(
    ...
    override fun open(provider: MapProvider, stationName: String, latitude: Double, longitude: Double) {
        val packageName = provider.packageName()
        val mapIntent = Intent(Intent.ACTION_VIEW, Uri.parse(provider.mapUri(stationName, latitude, longitude)))
...
private fun MapProvider.mapUri(
    ...
    return when (this) {
        ...
        MapProvider.KAKAO_NAVI -> "kakaomap://route?ep=$latitude,$longitude&ename=$encodedName&by=CAR"
