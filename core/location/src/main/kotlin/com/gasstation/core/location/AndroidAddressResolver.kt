...
internal fun interface GeocoderAsyncLookup {
    fun getFromLocation(latitude: Double, longitude: Double, maxResults: Int, listener: Geocoder.GeocodeListener)
}

@RequiresApi(Build.VERSION_CODES.TIRAMISU)
internal suspend fun GeocoderAsyncLookup.awaitFromLocation(latitude: Double, longitude: Double, maxResults: Int): List<Address>? =
...
@RequiresApi(Build.VERSION_CODES.TIRAMISU)
private class AndroidGeocoderAsyncLookup(private val geocoder: Geocoder) : GeocoderAsyncLookup {
...
internal class GeocoderLookupException(message: String?) : IOException(message ?: "Geocoder lookup failed")
