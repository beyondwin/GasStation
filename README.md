...
GitHub Actions `Android CI`는 PR에서 `agent-contracts`, `static-analysis`, `unit-tests`, `screenshot-tests`, `assemble`, `coverage`를 실행합니다. Coverage는 settings의 18개 명시 모듈과 demo/prod 보고서를 provider 기반으로 발견하고, authored production/test provenance와 모듈·상태 unit ratchet을 차단형으로 검사합니다. `assemble`은 demo/prod debug와 benchmark를 확인하고, `main`/`v*` tag push에서는 `release-assemble`도 추가 실행합니다. `v*` tag에서는 모든 job 성공 뒤 `release-publish`가 demo debug APK, unsigned prod release APK, SHA-256 checksum을 GitHub Release에 게시합니다.
